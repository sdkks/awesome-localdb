...
Usage: python src/recipe_sql_queries.py
"""
...
def main() -> None:
    db_path = os.path.join(os.path.dirname(__file__), "..", "recipe_sql_queries.db")
    ...
    # ── 4. Read: Select ───────────────────────────────────────────────
    rows = db.execute("SELECT id, title, author, year FROM books").fetchall()
    print(f"\nAll books ({len(rows)}):")
    for row in rows:
    ...
    try:
        ...
        remaining = db.execute("SELECT COUNT(*) FROM books").fetchone()[0]
